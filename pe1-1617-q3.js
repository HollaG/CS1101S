...
function evaluate_BAE_tree(bae_tree) {
    ...
    // WRITE HERE.
    if (!is_list(bae_tree) && !is_null(bae_tree)) {
        return bae_tree;
    }
    ...
    if (is_list(left) && is_list(right)) {
        ...
        if (op === "+") {
            return evaluate_BAE_tree(left) + (right);
        } else if (op === "-") {
            return evaluate_BAE_tree(left) - (right);
        } else if (op === "*") {
            return evaluate_BAE_tree(left) * (right);
        ...
        if (op === "+") {
            return (left) + evaluate_BAE_tree(right);
        } else if (op === "-") {
            return (left) - evaluate_BAE_tree(right);
        } else if (op === "*") {
            return (left) * evaluate_BAE_tree(right);
...
function build_BAE_tree(bae_list) {
    ...
    // WRITE HERE.
    // every ( is the start of a list, every ) is the end
    ...
    // everytime you see a (, create a new list 
    // everytime you see a ), need to somehow 'get out' of the list
    // otherwise, just pair it
    ...
    //         return null;
    //     } else if (head(lst) === "(") {
    //         return list(helper(tail(lst)));
            
    //     } else if (head(lst) === ")") {
    //         return helper(tail(lst));
    //     } else  {
    //         return pair(head(lst), helper(tail(lst)));
    //     }
    // };
    
    // const n = helper(bae_list);
    // display_list(n);
...
function check_parentheses(paren_list) {
    ...
    // WRITE HERE.
    // each (  must have at most one  )
    // method 1: count number of ( and ) 
    
    // keep a counter
    // everytime you see a (, add 1
    // everytime you see a ), subtract 1
