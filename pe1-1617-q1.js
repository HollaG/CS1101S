...
function find_gene_start(xs) {
    ...
    let first = head(tail(tail(xs)));
    let prev = head(tail(xs));
    let prevPrev = head(xs);
    function helper(xs) {
        if (first === "G" && prev === "T" && prevPrev === "A") {
            return list(xs);
        } else if (is_null(xs)) {
    ...
    return helper(tail(tail(tail(xs))));
...
function find_gene_end(xs) {
    ...
    let ahead_one = head(tail(xs));
    let ahead_two = head(tail(tail(xs)));
    let holder = null;
    // function helper(xs) {
    //     const now = head(xs);
    //     if (now === "")
    // }
    let not_found = true;
    for (let i = 0; i < length(xs) - 2; i = i + 1) {
        const now = list_ref(xs, i);
        const ahead_one = list_ref(xs, i + 1);
        const ahead_two = list_ref(xs, i + 2);
        
        const code = now + ahead_one + ahead_two;
        if (code === "TAG" || code === "TAA" || code === "TGA") {
            not_found = false;
            break;
        } else {
            holder = pair(now, holder);            
        }
        
    }

    return not_found ? null : list(reverse(holder));
...
function all_genes(xs) {
    ...
    function helper() {
        // display_list(list_holder);
        if (is_null(list_holder)) {
            return null;
        } else {
            // 1) find the start
            const start = find_gene_start(list_holder);
            
            if (is_null(start)) {
                // no start found
                return undefined;
            } else {
                list_holder = head(start);
