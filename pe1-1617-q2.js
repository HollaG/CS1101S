...
function all_different(nums) {
    ...
    // WRITE HERE.
    let seen_before = list();
    
    let unique = true;
    for (let i = 0; i < length(nums); i = i + 1) {
        const elem = list_ref(nums, i);
        
        if (!is_null(member(elem, seen_before))) {
            unique = false;
            break;
        } else {
            seen_before = append(seen_before, list(elem));
        }
...
function is_valid_toto_set(nums, n, min, max) {
    ...
    // 3) and 4), check min max 
    return accumulate((x, y) => y && x <= max && x >= min, true, nums);
...
function check_winning_group(bet_nums, draw_nums, extra_num) {
    ...
    // extra num will never be in the draw_nums
    // check if any in bet_nums is = extra_num
    const is_in_extra = is_null(member(extra_num, bet_nums)) ? false : true;
    if (is_in_extra) {
        total = total + 1;
    }
    
    // count how many in bet_nums are in draw_nums
    const num = num_of_matches(bet_nums, draw_nums);
    total = total + num;
    
    const total_to_hit = length(draw_nums);
    if (num === total_to_hit) {
        // group 1
        return 1;
    }
    if (num === total_to_hit - 1) {
        if (is_in_extra) {
            return 2;
        } else {
            return 3;
