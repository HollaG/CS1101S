...
function build_largest_int(digits) {
    ...
    sort_ascending(copy);
    let final = reverse_array(copy);
    let res = "";
    for (let i = 0; i < array_length(copy); i = i + 1) {
        res = res + stringify(copy[i]);
    }
    display(res);
    return res;
}
...
function build_2nd_largest_int(digits) {
    ...
    // WRITE HERE.
    // idea: make the largest int
    // then: keep the 1st (i = 0) element, 
    // and apply a function to sort the remaining ones
    
    
    // swap 1 and 10
    // if same, swap 10 and 100
    ...
    reverse_array(copy);
    // display_list(copy);
    let last_digit = copy[array_length(copy)-1];
    for (let i = array_length(copy) - 1; i >= 0; i = i - 1) {
        if (i === 0) {
